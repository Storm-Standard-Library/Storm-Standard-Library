...
Bitformatting = {
	...
	---@section flattenTable_SL
	---Only to be used with array-like tables, 1 indexed
	---@param tab table table to flatten
	---@return table
	flattenTable_SL = function(tab)
		local output, tableType, flatten_recursive = {}, 'table'
		function flatten_recursive(t)
			insert_SL(output, #t)
			local pos = 1
			while pos <= #t do

				local valueType, count = type_SL(t[pos]), 0
				repeat
					count = count + 1
				until type_SL(t[pos + count]) ~= valueType

				if valueType == tableType then
					insert_SL(output, (count << 1) + 1)
					for i = pos, pos + count - 1 do
						flatten_recursive(t[i])
					end
				else
					insert_SL(output, count << 1)
					for i = pos, pos + count - 1 do
						insert_SL(output,t[i])
					end
				end
				pos = pos + count
			end
	...
	---@section unflattenTable_SL
	---Returns an unflattened array-like 1 indexed table
	---@param flat table flattened table
	---@return table
	unflattenTable_SL=function(flat)
		local function recursive(pos)
			local t, inserted, n, header, count, subTable = {}, 0, flat[pos]
			pos = pos + 1
			while inserted < n do
				header = flat[pos]
				count = header >> 1
				...
				if header & 1 == 1 then
					for i = 1, count do
						...
						insert_SL(t, subTable)
					end
				else
					for i = 1, count do
						insert_SL(t, flat[pos])
					end
					pos = pos + count
				end
